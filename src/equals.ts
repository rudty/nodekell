...
import { undefinedValue, _isObjectArray, _isTypedArray, _isPrimitiveWrapper, _isFunction } from "./internal/typeTraits";
import { underBar } from "./_";
export interface EqualsType {
    <T>(a: T, b: T): boolean;
    <T>(a: T): (b: T) => boolean;
}

let _equals: EqualsType;

const map_internal = (lhs: Map<any, any>, rhs: Map<any, any>): boolean => {
...
const set_internal = (lhs: Set<any>, rhs: Set<any>): boolean => {
    if (lhs.size !== rhs.size) {
        return false;
    }
...
const regExp_internal = (lhs: RegExp, rhs: RegExp): boolean => {
...
const typedArray_internal = (lhs: ArrayLike<any>, rhs: ArrayLike<any>) => {
...
const array_internal = (lhs: ArrayLike<any>, rhs: ArrayLike<any>) => {
    const len = lhs.length;
    if (len !== rhs.length) {
        return false;
    }
...
const object_internal = (lhs: any, rhs: any) => {
...
const toPrimitive_call_internal = (a: any, toPrimitiveFunc: any, hint: any): any => {
    try {
...
const toPrimitive_internal = (a: any): any => {
...
const toString_internal = (a: any): string => ((<any> (Object.prototype.toString.call))(a));

_equals = <any> curry((lhs: any, rhs: any) => {
    if (lhs === rhs) {
