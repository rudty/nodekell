...
describe('test sort2', () => {
    ...
    it('number array', async () => {
        ...
        const r0 = await F.sortBy(asc,  a);
        const r1 = await F.sortBy((a,b) => b-a)(a);
    ...
    it('type array sort', async () => {
        const a = await F.run(
            F.repeat(300, () => F.random(9999)),
            F.reverse,
            F.collectInt32);

        const r0 = Int32Array.from(a).sort(asc);
        const r1 = await F.sortBy(asc, a);
        assert.deepStrictEqual(r0, r1);
    });
