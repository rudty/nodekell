...
const getFirstKeyValue = <K, V>(m: Map<K, V>): [K, V] => {
    return [getFirstKey(m), getFirstValue(m)];
};
...
describe('innerJoin', () => {
    it('from Map Array', async () => {
        ...
        const m0 = rr0[0];
        const kv0 = getFirstKeyValue(m0); // $ExpectType [string, string | number]

        const rr1 = await F.collect(r1);
        const m1 = rr1[0];
        const kv1 = getFirstKeyValue(m1); // $ExpectType [string, string | number]

        const rr2 = await F.collect(r2);
        const m2 = rr2[0];
        const kv2 = getFirstKeyValue(m2); // $ExpectType [string, string | number]

        const rr3 = await F.collect(r3);
        const m3 = rr3[0];
        const kv3 = getFirstKeyValue(m1); // $ExpectType [string, string | number]
    });
    ...
    it('with run', async () => {
        ...
        const kv1 = getFirstKeyValue(r1[0]); // $ExpectType [string, string | number]
    });
});
...
describe('leftInnerJoin', () => {
    it('from Map Array', async () => {
        ...
        const rr0 = await F.collect(r0);
        const kv0 = getFirstKeyValue(rr0[0]); // $ExpectType [string, string | number]

        const rr1 = await F.collect(r1);
        const kv1 = getFirstKeyValue(rr1[0]); // $ExpectType [string, string | number]

        const rr2 = await F.collect(r2);
        const kv2 = getFirstKeyValue(rr2[0]); // $ExpectType [string, string | number]

        const rr3 = await F.collect(r3);
        const kv3 = getFirstKeyValue(rr3[0]); // $ExpectType [string, string | number]
    });
...
describe('rightInnerJoin', () => {
    it('from Map Array', async () => {
        ...
        const rr3 = await F.collect(r3);
        const m3 = rr3[0];
        const kv3 = getFirstKeyValue(m3); // $ExpectType [string, string | number]
    });
